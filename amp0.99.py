...
#                             v0.99                               #
#                                                                 #
#             Jeremy M. Brown and Bradley J. Nelson               #
#                     jeremymbrown@gmail.com                      #
#                                                                 #
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    """
    ...
    """.format(NPROCS = NPROCS, NWORKERS = NWORKERS)
    ...
    if args.timeit:
        ...
        Time spent calculating test statistics: %f s, %f proc-s""" % (read_time / NPROCS, read_time, 
                                                                      calc_time / NPROCS, calc_time)
...
def worker(args, in_queue, out_queue):
    ...
    for rep in iter(in_queue.get, 'STOP'):
...
def pp_calc(args, i):
    ...
    if (args.quantile or args.entropy or args.iqr or len(args.part_list) > 0 or args.length or args.tlVar):
    ...
    if args.quantile or args.iqr or args.entropy:
        (pp_rf_dists, pp_topo_freqs) = calcAndSortTreeDists(args, dendropy.DataSet(pp_tree_list))
    ...
    if args.entropy:
        pp_entropies = entropy_test(args, dendropy.DataSet(pp_tree_list), pp_topo_freqs)
    ...
    if len(args.part_list) > 0:
        pp_bpps = partition_test(args, dendropy.DataSet(pp_tree_list))
    else:
        pp_bpps = []
    ...
    if args.length:
        pp_tls = treelength_test(args, dendropy.DataSet(pp_tree_list))
    else:
        pp_tls = []

    # Calculate treelength variance test statistic
    if args.tlVar:
        pp_tlVar = tlVar_test(args, dendropy.DataSet(pp_tree_list))
    else:
        pp_tlVar = []
...
def getMrCburn(args, i):
    ...
    try:
        ...
        mrcin = open(file, 'r')                         # Opens log file for reading
...
def calcAndSortTreeDists(args, tree_data):
    ...
    try:
        ...
        tree_list = list(reversed(range(len(trees))))
        ...
        while len(tree_list) > 1:
            ...
            for j in tree_list:
                ...
                rf_dist = symm_diff(trees[i],trees[j])
                current_rfs.append(rf_dist)
                if rf_dist == 0:
                    duplicate_trees.append(j)
...
def entropy_test(args, trees, topo_freqs):
    ...
    try:
        ...
        entropy = entropy + math.log(float(ntopologies))
...
def partition_test(args, tree_data):
    ...
    try:
        trees = tree_data.tree_lists[0]
        p = trees.frequency_of_split(labels=args.part_list)
        if p < 1 and p > 0:
...
def tlVar_test(args, tree_data):
    ...
    try:
        tls = []
        trees = tree_data.tree_lists[0]
        [tls.append(tree.length()) for tree in trees]
